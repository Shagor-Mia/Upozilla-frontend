...
import { HospitalManageForm } from "@/components/hospitals/HospitalManageForm";
import { TrustGate } from "@/components/listings/TrustGate";
import { getLocationOptions } from "@/lib/locations";
import { getSession } from "@/lib/session";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("sellHospital");
  return { title: t("heading") };
}

export default async function SellHospitalPage() {
  const [t, session, locations] = await Promise.all([
    getTranslations("sellHospital"),
    getSession(),
    getLocationOptions(),
  ]);
  ...
  return (
    <div>
      ...
      {session && !session.canManageHospital ? (
        ...
        <TrustGate locked={!session} reason="signin" className="mt-6">
          <div className="max-w-2xl rounded-xl border border-border-muted bg-surface-container-lowest p-6 shadow-card">
            <HospitalManageForm locations={locations} />
          </div>
        </TrustGate>
      )}
    </div>
