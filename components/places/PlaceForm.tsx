...
/** Public submission form (POST /places/submit) - any logged-in user, no
 * phone verification. Always enters the moderation queue pending; the page
 * itself gates on sign-in only via TrustGate, unlike ListingForm's phone
 * verification gate. */
export function PlaceForm({ categories, locations }: { categories: Option[]; locations: Option[] }) {
  ...
  async function submit(payload: Record<string, unknown>) {
    await run(() => clientApi.post<Place>("places/submit", payload), {
      onSuccess: () => {
        router.push("/account/places?submitted=1");
        router.refresh();
      },
